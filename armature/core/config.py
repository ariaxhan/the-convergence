...
class ArmatureConfig(BaseSettings):
    """
    Main configuration for The Armature framework.
    ...
    generator_template_dir: Path = Field(
        default=Path("./armature/templates"),
    ...
    @classmethod
    def from_yaml(cls, path: Path) -> "ArmatureConfig":
    ...
    @classmethod
    def from_json(cls, path: Path) -> "ArmatureConfig":
...
_global_config: Optional[ArmatureConfig] = None


def get_config() -> ArmatureConfig:
    """Get the global configuration."""
    global _global_config
    if _global_config is None:
        _global_config = ArmatureConfig()
    return _global_config


def set_config(config: ArmatureConfig) -> None:
