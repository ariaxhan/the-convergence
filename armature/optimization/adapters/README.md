...
Armature uses an **adapter pattern** to support multiple API providers without hardcoding provider-specific logic in the core optimization engine.
...
Create `armature/optimization/adapters/your_provider.py`:
...
Edit `armature/optimization/runner.py`:
...
```python
from armature.optimization.adapters.your_provider import YourProviderAdapter
from armature.optimization.models import APIResponse
