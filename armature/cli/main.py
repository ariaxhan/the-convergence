"""
Command-line interface for The Armature API Optimization Framework.
"""
...
logging.getLogger("armature.storage").setLevel(logging.INFO)
logging.getLogger("armature.storage.sqlite_backend").setLevel(logging.INFO)
logging.getLogger("armature.storage.file_backend").setLevel(logging.INFO)
...
try:
    ...
    weave_project = os.getenv("WEAVE_PROJECT") or os.getenv("WANDB_PROJECT", "armature")
...
app = typer.Typer(
    name="armature",
    help="The Armature: API Optimization Framework powered by agent society"
)
...
try:
    from armature.generator import initialize_project
...
def initialize_weave(config) -> bool:
    ...
    weave_project = weave_project or os.getenv("WEAVE_PROJECT") or os.getenv("WANDB_PROJECT", "armature-optimization")
...
def init(
    ...
    """
    Initialize Armature project with interactive setup.
    ...
    Example:
        armature init                    # Interactive setup
        armature init --output ./config  # Custom output dir
    """
...
def _check_and_prompt_api_keys(config) -> None:
    ...
    if provide_now:
        ...
        console.print(f"  [dim]armature optimize {config.api.name}_optimization.yaml[/dim]\n")
        raise typer.Exit(1)
...
def optimize(
    ...
    Example:
        ...
        armature optimize browserbase.yaml --output ./results/
...
"""
    ...
    report += "\n## Summary\n\n"
    report += "*Optimized by The Armature agent society (MAB + Evolution + RL)*\n"
...
**Generated by The Armature** - API Optimization Framework
...
@app.command()
def info():
    """Show information about The Armature framework."""
    console.print(Panel(
        "[bold cyan]The Armature[/bold cyan]\n"
        ...
        "[bold]Docs:[/bold] github.com/ariaxhan/armature-ai",
        title="📚 About",
        border_style="cyan"
    ))
...
def version():
    ...
    console.print(f"[bold cyan]The Armature[/bold cyan] v{__version__}")
