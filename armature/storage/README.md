# Armature Storage Backends

Pluggable storage system for Armature optimization framework.
...
```python
from armature.storage import ConvexStorage
from armature import ArmatureSDK
...
# Use with Armature
sdk = ArmatureSDK(storage=storage)
...
class MyConvexClient:
    ...
    async def save_rl_data(self, **kwargs):
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{self.url}/api/armature/storage/rl-data",
                json=kwargs,
    ...
    async def save_experiment(self, **kwargs):
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{self.url}/api/armature/storage/experiments",
                json=kwargs,
                headers=self.headers
            )
            return response.json()
...
```python
from armature.storage import get_legacy_storage
...
```python
from armature.storage import StorageProtocol
