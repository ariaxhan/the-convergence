"""
Convex Storage Backend for The Armature

Generic bridge that routes Armature storage calls to any Convex implementation.
...
class ConvexStorage:
    """
    Convex storage backend - routes Armature data to Convex.
    ...
    def __init__(
        ...
        - app.convex_toolkit.api.armature_storage_convex (if available)
        ...
        # Option 3: Auto-import from standard location
            ...
            try:
                from app.convex_toolkit.api import armature_storage_convex
                self.storage = armature_storage_convex
                self._custom_fns = None
        ...
        if value.get("status") == "started":
            ...
            if self._custom_fns and self._custom_fns.get('complete_run'):
                response = await self._custom_fns['complete_run'](
                    ...
                    armature_achieved=value.get("armature_achieved"),
                )
            else:
