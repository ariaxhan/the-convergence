...
async def create_preset_config(
    ...
    # Find the armature package root
    import armature
    armature_root = Path(armature.__file__).parent.parent

    # Source files (the working examples)
    source_config = armature_root / "examples" / template["config"]
    source_tests = armature_root / "examples" / template["tests"]
    source_evaluator = armature_root / "examples" / template["evaluator"]

    # Optional runner file (for agent templates: Reddit, Gmail, Discord)
    source_runner = None
    if "runner" in template:
        source_runner = armature_root / "examples" / template["runner"]

    # Base agent runner for agent templates (Discord, Gmail, Reddit)
    source_base_runner = None
    if preset_name in ["reddit", "gmail", "discord"]:
        source_base_runner = armature_root / "examples" / "agno_agents" / "base_agent_runner.py"
    ...
    console.print("  3. Run: armature optimize optimization.yaml")
    console.print("")
