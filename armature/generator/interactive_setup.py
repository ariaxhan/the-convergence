"""
Interactive setup for Armature when no OpenAPI spec is found.
...
async def _gather_config_preferences(console: Console, template: Dict[str, Any], skip_api_key: bool = False) -> Dict[str, Any]:
    ...
    if not skip_api_key:
        ...
        if _looks_like_api_key(key_env):
            console.print("")
            console.print("[bold red]❌ ERROR: You entered what looks like an actual API key![/bold red]")
            console.print("[yellow]   Please enter the ENVIRONMENT VARIABLE NAME, not the key value.[/yellow]")
        ...
        if not os.getenv(key_env):
            ...
            console.print("[dim]   Before running optimization, set it with:[/dim]")
