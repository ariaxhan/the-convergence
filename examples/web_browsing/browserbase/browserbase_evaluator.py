    ...
    try:
        ...
        if isinstance(result, str):
            ...
            try:
                ...
                logger.warning("🔄 Falling back to MOCK evaluation (JSON parse error)")
            ...
            logger.warning("🔄 Falling back to MOCK evaluation (unknown format)")
            return _evaluate_mock_result(result, expected, params, metric)

        # Check if this looks like a BrowserBase response
        logger.debug("🔍 Checking for BrowserBase response markers...")
        ...
        if isinstance(data, dict) and (has_status or has_data or has_session_id):
            logger.info("✨ REAL BROWSERBASE RESPONSE DETECTED")
            ...
            logger.warning("⚠️  Response doesn't match BrowserBase format")
            logger.warning(f"   Dict keys found: {list(data.keys()) if isinstance(data, dict) else 'N/A'}")
            logger.warning("🔄 Falling back to MOCK evaluation (no status/data fields)")
        ...
        logger.error("🔄 Falling back to FALLBACK evaluation (exception)")
...
def _evaluate_real_browserbase_result(data: Dict[str, Any], expected: Any, params: Dict[str, Any], metric: Optional[str]) -> float:
        ...
        if status == 'success' or status == 'completed':
            ...
            if isinstance(expected, dict):
                ...
                if isinstance(scraped_data, list) and len(scraped_data) >= min_elements:
                    ...
                    logger.info("📈 REAL success_rate score: 0.7 (partial data)")
                    return 0.7
    ...
    logger.info("📈 REAL default score: 0.6 (unknown metric)")
    return 0.6
...
def _evaluate_mock_result(result: Any, expected: Any, params: Dict[str, Any], metric: Optional[str]) -> float:
    ...
    # Default score
    logger.info("📈 MOCK default score: 0.6000")
    return 0.6
...
def _evaluate_fallback(params: Dict[str, Any], metric: Optional[str]) -> float:
    ...
    if (browser_type == 'chromium' and headless and
        ...
        logger.info("📈 FALLBACK score: 0.85 (optimal config)")
        return 0.85

    # Good combination
    elif (browser_type in ['chromium', 'firefox'] and headless and
          1280 <= viewport_width <= 2560 and 3000 <= timeout_ms <= 30000):
        logger.info("📈 FALLBACK score: 0.70 (good config)")
        return 0.70

    # Decent combination
    elif browser_type in ['chromium', 'firefox'] and 1000 <= timeout_ms <= 30000:
        logger.info("📈 FALLBACK score: 0.55 (decent config)")
        return 0.55

    # Fallback
    else:
        logger.info("📈 FALLBACK score: 0.45 (default)")
        return 0.45
