...
from convergence.core.weave_logger import init_weave_logger
...
class CivilizationRuntime:
    ...
    def __init__(
        ...
        if self.weave_logger and self.weave_logger.enabled:
            ...
            console.print("[dim yellow]⚠️  Weave tracing disabled[/dim yellow]")
            if not os.getenv("WANDB_ENTITY") and not os.getenv("WEAVE_ORGANIZATION"):
                console.print("[dim yellow]   Set WANDB_ENTITY in .env to enable tracing[/dim yellow]")
