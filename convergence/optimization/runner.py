...
from .evolution import ConfigurationAnalyzer, EvolutionEngine
from .models import APIResponse, OptimizationResult, OptimizationSchema
from .response_utils import extract_response_text
from .rl_optimizer import RLMetaOptimizer
from .test_case_evolution import TestCaseAnalyzer, TestCaseEvolutionEngine
...
class OptimizationRunner:
    ...
    def __init__(
        ...
        if config.society and config.society.enabled:
            ...
            if config.society.learning.rlp_enabled:
                from ..plugins.learning.rlp import RLPConfig, RLPMixin
            ...
            if config.society.learning.sao_enabled:
                from ..plugins.learning.sao import SAOConfig, SAOMixin
    ...
    def _init_legacy(self) -> None:
        ...
        from convergence.legacy import LegacyConfig, LegacyStore
    ...
    async def _load_warm_start_configs(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if winners:
                ...
                print("   └─ Note: Scores shown are each config's BEST performance on a specific test")
                print("   └─ Overall scores (averaged across all tests) will be calculated below\n")
    ...
    async def run(self) -> OptimizationResult:
        ...
        if num_warm > 0:
            ...
            print("   💡 Tip: Mixing proven + new configs helps find better solutions faster!")
            ...
            print("   └─ No previous data found - starting fresh exploration\n")
        ...
        if self.config.legacy and self.config.legacy.enabled:
            ...
            print("   💡 Next Run: Will load these winning configs as warm-start!")
        else:
            print("   └─ (Legacy tracking disabled - no warm-start available)\n")
    ...
    async def _call_api(
        ...
        if self.config.api.auth.type == "api_key":
            ...
            if token:
                auth_config = {"type": "bearer", "token_env": token_env}
    ...
    async def _record_to_legacy(self, result: OptimizationResult) -> None:
        ...
        from convergence.legacy.models import OptimizationRun, TestCaseResult
        ...
        try:
            ...
            async with self.legacy_store:
                ...
                print("   └─ Status: Accumulating history across all runs\n")
                ...
                print("✅ Legacy tracking complete")
                print(f"   📊 Session '{self.legacy_session.session_id}':")
